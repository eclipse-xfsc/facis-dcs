...
def step_then_negotiation_interface_displayed(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('I can view all contract clauses')
def step_then_view_clauses(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('only the targeted clause is changed')
def step_then_targeted_clause_changed(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the rest of the contract remains unchanged')
def step_then_rest_unchanged(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the document integrity is maintained')
def step_then_document_integrity_maintained(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the contract is routed to required approvers')
def step_then_routed_to_approvers(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the contract status shows "Pending Approval"')
def step_then_status_pending_approval(context):
    assert context.requests_response.status_code == 200, context.requests_response.text
...
def step_then_approval_logged_credentials(context):
    assert context.requests_response.status_code == 200, context.requests_response.text
...
def step_then_contract_back_to_draft(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the contract is returned for revision')
def step_then_contract_returned_for_revision(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the contract is routed to assigned reviewers')
def step_then_routed_reviewers(context):
    assert context.requests_response.status_code == 200, context.requests_response.text


@then('the contract status changes to "Under Review"')
def step_then_contract_under_review(context):
    assert context.requests_response.status_code == 200, context.requests_response.text
